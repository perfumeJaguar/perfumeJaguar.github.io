/** DODREI — MEMORY RECALL PROTOTYPE v1.0.25
 * Deterministically maps every archive image to a placeholder memory fragment.
 * Holding the image for 1s reveals a full-screen recall plate containing only
 * the held archive image as an unfiltered thumbnail plus its memory text.
 */
(()=>{
  const HOLD_MS=1000;
  const fragments=[
  ...
  let timer=null,heldKey=null,visible=false,showToken=0;
  ...
  function end(){
    clearTimeout(timer);timer=null;heldKey=null;showToken+=1;
